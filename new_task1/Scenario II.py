...
for del_net in del_net1:
    for i in range(len(dyn_names)):
        ...
        for k in range(10):   
            ...
            print(acc_state/(batch*batch_num*len(s1[0])))
            result1.append([acc3/(len(w1)*z),loss_state.item(),acc_state/(batch*batch_num*len(s1[0])),loss_state1.item(),acc_state1/(batch*len(s1[0]))])
        np.savetxt("pspd"+net_path+'_'+dyn_name+"_del_ding"+str(int(del_net*100))+'_'+str(k)+".txt", np.array(result), fmt="%.6f", delimiter=" ")
        np.savetxt("pspd"+net_path+'_'+dyn_name+"_del_xiao"+str(int(del_net*100))+'_'+str(k)+".txt", np.array(result1), fmt="%.6f", delimiter=" ")
