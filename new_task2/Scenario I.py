...
for del_net in del_net1:
    for i in range(len(dyn_names)):
        ...
        for k in range(10):   
            ...
            print(acc3/z)
            result1.append([acc3/(z),0])
        np.savetxt(net_path+'_'+dyn_name+"_del_gin_ding"+str(int(del_net*100))+'_'+str(k)+".txt", np.array(result), fmt="%.6f", delimiter=" ")
        np.savetxt(net_path+'_'+dyn_name+"_del_gin_xiao"+str(int(del_net*100))+'_'+str(k)+".txt", np.array(result1), fmt="%.6f", delimiter=" ")